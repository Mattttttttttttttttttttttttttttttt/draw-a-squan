...
async function browserSmoke(options = {}) {
    ...
    function assertClipboard(record, expected) {
    ...
    async function runCase(name, expected, fn) {
        try {
            ...
            results.push({
                ...
                extension: expected === 'jpeg' ? 'jpg' : expected,
    ...
    async function runClipboardCase(name, expected, fn) {
        try {
            const record = await fn();
            assertClipboard(record, expected);
            results.push({
                name,
                kind: 'clipboard',
                size: record.size,
                type: record.type,
                filename: record.filename,
                extension: expected === 'svg' ? 'svg' : 'png',
                base64: record.base64,
            });
        } catch (err) {
            failures.push({ name, error: String(err?.message || err) });
            ...
            for (const layer of layerList) {
                ...
                if (clipboardExpected) {
                    if (!copyButtonVisible()) {
                        failures.push({ name: `${style.name} clipboard ${format} ${layer}`, error: 'Copy button is not visible' });
                    } else {
                        await runClipboardCase(`${style.name} clipboard ${format} ${layer}`, clipboardExpected, () =>
                            takeClipboard(() => {
                    ...
                    failures.push({
                        name: `${style.name} clipboard ${format} ${layer}`,
                        error: 'Copy button is visible for an unsupported clipboard format',
                    });
                } else {
                    skipped.push(`${style.name} clipboard ${format} ${layer} unsupported`);
                }
            }
        }
